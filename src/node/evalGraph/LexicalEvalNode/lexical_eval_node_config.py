...
See lexical_eval_node_helper for details.
"""

from typing import Any, Dict, Optional

from node.evalGraph.LexicalEvalNode.lexical_eval_node_helper import (
    build_framework_criterion_context,
    build_question_context,
    run_eval_node,
)
from node.evalGraph.LexicalEvalNode.lexical_eval_prompt import SYSTEM_PROMPT
...
def build_user_prompt(speaking_input: SpeakingInput, transcript: str, metrics: Optional[Dict[str, Any]] = None) -> str:
    ...
    if metrics:
        ...
        parts.append(f"Lexical range cap: {metrics.get('lexical_range_cap')}")
...
def lexical_eval_node(state: dict) -> dict:
    return run_eval_node(
        state, SYSTEM_PROMPT, build_user_prompt, "lexical",
        result_key="lexical_criterion", confidence_key="vocabulary_confidence",
    )
